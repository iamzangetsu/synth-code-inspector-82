...
function detectLovableGeneration(metadata: any, commits: any[], allFiles: GitHubFile[]): { isLovable: boolean; indicators: string[] } {
  ...
  // Check README for Lovable mentions
  const readmeFile = allFiles.find(file => file.name.toLowerCase() === 'readme.md');
  if (readmeFile) {
    // We would need to fetch and check README content, but for now just note its presence
    indicators.push('README.md present (content not checked)');
  }
...
export async function analyzeGitHubRepository(
  ...
  const lovableDetection = detectLovableGeneration(metadata, commits, allFiles);
